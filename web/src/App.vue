...
window.addEventListener("hashchange", () => { route.value = location.hash.replace("#", "") || "/programs"; if (route.value === "/recommend" && !groupedPrograms.value.length) loadPrograms(); });
...
async function getRecommendations() {
  isRecommending.value = true; recommendError.value = "";
  const requestProfile = { estimated_score: profile.value.estimatedScore, target_major: profile.value.targetMajor, preferred_provinces: profile.value.preferredProvinces.split(",").map(x => x.trim()).filter(Boolean), risk_preference: profile.value.riskPreference };
  try {
    const matchParams = new URLSearchParams();
    if (requestProfile.preferred_provinces.length === 1) matchParams.set("province", requestProfile.preferred_provinces[0]);
    if (requestProfile.target_major) matchParams.set("majorCode", requestProfile.target_major.substring(0, 2));
    const matchRes = await fetch(`${apiBase}/programs/match?${matchParams}`);
    if (!matchRes.ok) throw new Error("数据查询失败 " + matchRes.status);
    const matched = await matchRes.json();
    if (!matched.length) { recommendError.value = "未找到匹配的院校数据，请调整筛选条件"; recommendations.value = []; return; }
    const res = await fetch(recommendationUrl, { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({profile:requestProfile, programs:matched, weights:weights.value}) });
    if (!res.ok) throw new Error("推荐服务返回错误 " + res.status);
    const body = await res.json();
    recommendations.value = body.items || [];
    if (!recommendations.value.length) recommendError.value = "推荐引擎未返回结果";
  } catch(e) { recommendError.value = "推荐失败：" + e.message; recommendations.value = []; } finally { isRecommending.value = false; }
}
